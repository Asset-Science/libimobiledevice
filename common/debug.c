 ...
 */
 ...
 static void debug_print_line(const char *func, const char *file, int line, const char *buffer)
 {
	 ...
	 /* print header */
	 fprintf(mStderr != NULL ? mStderr : stderr, "%s: ", header);
 
	 /* print actual debug content */
	 fprintf(mStderr != NULL ? mStderr : stderr, "%s\n", buffer);
 ...
 void debug_buffer(const char *data, const int length)
 {
	 ...
	 if (debug_level) {
		 for (i = 0; i < length; i += 16) {
			 fprintf(mStderr != NULL ? mStderr : stderr, "%04x: ", i);
			 for (j = 0; j < 16; j++) {
				 if (i + j >= length) {
					 fprintf(mStderr != NULL ? mStderr : stderr, "   ");
					 continue;
				 }
				 fprintf(mStderr != NULL ? mStderr : stderr, "%02x ", *(data + i + j) & 0xff);
			 }
			 fprintf(mStderr != NULL ? mStderr : stderr, "  | ");
			 for (j = 0; j < 16; j++) {
				 if (i + j >= length)
				 ...
				 fprintf(mStderr != NULL ? mStderr : stderr, "%c", c);
			 }
			 fprintf(mStderr != NULL ? mStderr : stderr, "\n");
		 }
		 fprintf(mStderr != NULL ? mStderr : stderr, "\n");
