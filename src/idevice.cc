 ...
 */
 ...
 static void usbmux_event_cb(const usbmuxd_event_t* event, void* user_data)
 {
	 ...
	 ev.conn_type = (idevice_connection_type)0;
 ...
 LIBIMOBILEDEVICE_API idevice_error_t idevice_events_subscribe(idevice_subscription_context_t *context, idevice_event_cb_t callback, void *user_data)
 {
	 ...
	 *context = (idevice_subscription_context_t)malloc(sizeof(struct idevice_subscription_context));
	 if (!*context) {
 ...
 LIBIMOBILEDEVICE_API idevice_error_t idevice_get_device_list_extended(idevice_info_t **devices, int *count, bool* stop)
 {
	 ...
	 *count = 0;
	 ...
	 if (usbmuxd_get_device_list(&dev_list, stop) < 0 || dev_list == NULL) {
	 ...
	 for (i = 0; dev_list[i].handle > 0; i++) {
		 newlist = (idevice_info_t *)realloc(*devices, sizeof(idevice_info_t) * (newcount+1));
		 newlist[newcount] = (idevice_info_t)malloc(sizeof(struct idevice_info));
		 newlist[newcount]->udid = _strdup(dev_list[i].udid);
 ...
 {
	 internal_set_stderr((FILE*) stream);
 }
 ...
 static idevice_t idevice_from_mux_device(usbmuxd_device_info_t *muxdev)
 {
	 ...
	 device->udid = _strdup(muxdev->udid);
	 ...
	 default:
		 device->conn_type = (idevice_connection_type)0;
		 device->conn_data = NULL;
		 break;
	 }
	 return device;
 ...
 LIBIMOBILEDEVICE_API idevice_error_t idevice_new_with_options(idevice_t * device, const char *udid, enum idevice_options options, bool* stop)
 {
	 ...
	 int res = usbmuxd_get_device(udid, &muxdev, (usbmux_lookup_options)usbmux_options, stop);
	 if (res > 0) {
 ...
 {
	 return idevice_new_with_options(device, udid, (idevice_options)0, stop);
 }
